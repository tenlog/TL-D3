...
static void lcd_dualxworkmode_menu();
#endif
...
static void lcd_main_menu()
{
    ...
    if (movesplanned() || IS_SD_PRINTING)
    {
        ...
        #ifdef DUAL_X_CARRIAGE
        MENU_ITEM(submenu, "Operation mode", lcd_dualxworkmode_menu);        
        #endif
...
static void lcd_tune_menu()
{
    ...
    MENU_ITEM_EDIT(int3, MSG_BED, &target_temperature_bed, 0, BED_MAXTEMP);
#endif
...
static void lcd_dualxworkmode_menu(){
    ...
    if(extruder_carriage_mode == 1)
        MENU_ITEM(gcode, "Dual color mode*", PSTR("M605 S1"));
    else
        MENU_ITEM(gcode, "Dual color mode", PSTR("M605 S1"));
    if(extruder_carriage_mode == 2)
        MENU_ITEM(gcode, "Duplication mode*", PSTR("M605 S2"));
    else
        MENU_ITEM(gcode, "Duplication mode", PSTR("M605 S2"));

    if(extruder_carriage_mode == 3)
        MENU_ITEM(gcode, "Mirror mode*", PSTR("M605 S3"));
    else
        MENU_ITEM(gcode, "Mirror mode", PSTR("M605 S3"));
    END_MENU();        
}
...
#endif
    MENU_ITEM(gcode, MSG_DISABLE_STEPPERS, PSTR("M84 X Y"));
    MENU_ITEM(gcode, MSG_AUTO_HOME, PSTR("G28"));
    //MENU_ITEM(gcode, MSG_SET_ORIGIN, PSTR("G92 X0 Y0 Z0"));

#ifdef DUAL_X_CARRIAGE
    if(active_extruder == 1)
        MENU_ITEM(gcode, "Switch to Nozzle1", PSTR("T0"));  
...
static void lcd_move_x()
{
    if (encoderPosition != 0)
    {
        ...
        if (min_software_endstops && current_position[X_AXIS] < fXMin)
            current_position[X_AXIS] = fXMin;
        if (max_software_endstops && current_position[X_AXIS] > fXMax)
            current_position[X_AXIS] = fXMax;
        /*
...
static void lcd_control_motion_menu()
{
    ...
    MENU_ITEM_EDIT(float52, MSG_ESTEPS, &axis_steps_per_unit[E_AXIS], 5, 9999);    
