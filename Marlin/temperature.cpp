...
static int bed_minttemp_raw = HEATER_BED_RAW_LO_TEMP; /* No bed mintemp error implemented?!? */ //By zyf
...
int watch_start_temp_fall[EXTRUDERS] = ARRAY_BY_EXTRUDERS(0,0,0);
unsigned long watchmillis_fall[EXTRUDERS] = ARRAY_BY_EXTRUDERS(0,0,0);
...
#ifdef BED_MINTEMP
  /* No bed MINTEMP error implemented?!? */  //By zyf
...
//      break;
  ...
  if(temp_count >= 16) // 8 ms * 16 = 128ms.
  {
    ...
    if(current_temperature_raw[0] <= minttemp_raw[0]) {
#endif
        if(target_temperature[0]>0){    //by zyf
            min_temp_error(0);
        }      
    }
#if EXTRUDERS > 1
    ...
    if(current_temperature_raw[1] <= minttemp_raw[1]) {
#endif
        if(target_temperature[1]>0){        //by zyf
            min_temp_error(1);
        }
    }
#endif
...
# if HEATER_BED_RAW_LO_TEMP > HEATER_BED_RAW_HI_TEMP
    ...
    if(current_temperature_bed_raw <= bed_minttemp_raw) {
#endif
       if(target_temperature_bed > 0){      //by zyf
           target_temperature_bed = 0;
           bed_min_temp_error();
        }
    }
